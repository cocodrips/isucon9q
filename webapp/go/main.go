...
type Category struct {
	...
	ParentCategoryName string `json:"parent_category_name,omitempty" db:"parent_name"`
}
...
func getNewCategoryItems(w http.ResponseWriter, r *http.Request) {
	...
	if itemID > 0 && createdAt > 0 {
		// paging
		// TODO statusとcategory_idにindexがあるか
...
func getItem(w http.ResponseWriter, r *http.Request) {
	...
	if (user.ID == item.SellerID || user.ID == item.BuyerID) && item.BuyerID != 0 {
		...
		if transactionEvidence.ID > 0 {
			shipping := Shipping{}
			// TODO transaction_evidence_idにindex
...
func postShip(w http.ResponseWriter, r *http.Request) {
	...
	transactionEvidence := TransactionEvidence{}
	// TODO transaction_evidencesにitem_idのindexはる
